...
class HealthSyncService {
      ...
      for (var data in healthData) {
        ...
        switch (data.type) {
          ...
          case HealthDataType.SLEEP_SESSION:
            final duration =
                data.dateTo.difference(data.dateFrom).inMinutes / 60.0;
            dailyData[date]!['sleep'] =
                (dailyData[date]!['sleep'] ?? 0.0) + duration;
            break;
          default:
            break;
        }
  ...
  Future<void> syncWorkouts(DateTime start, DateTime end) async {
    ...
    try {
      ...
      debugPrint(
        "DEBUG: Workouts found between $start and $end: ${workouts.length}",
      );
      ...
      for (var data in workouts) {
        debugPrint(
          "DEBUG: Processing data point: type=${data.type}, valueType=${data.value.runtimeType}",
        );
        ...
        if (data.type == HealthDataType.WORKOUT) {
          ...
          final isToday =
              data.dateFrom.year == DateTime.now().year &&
          ...
          if (shouldAutoImport) {
            ...
            if (!exists) {
              ...
              debugPrint(
                "Auto-imported workout: $activityType at ${data.dateFrom}",
              );
            }
          }
        }
      ...
      if (end.difference(start).inHours < 24 &&
        ...
        final todayKey =
            'daily_ride_notified_${DateTime.now().year}_${DateTime.now().month}_${DateTime.now().day}';

        if (dailyCyclingDistanceKm >= minKm) {
          if (prefs.getBool(todayKey) != true) {
            await NotificationService().showNewRideNotification(
              dailyCyclingDistanceKm,
              "Ciclismo",
            );
