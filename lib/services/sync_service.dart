...
class SyncService {
  ...
  static const _stravaClientSecret = 'b8618f582c8536e0a8202a8f68c74631f6d994ae'; // REPLACE ME
  ...
  Future<void> _exchangeCodeForTokenRaw(String code) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (data.containsKey('expires_at')) {
        ...
        final expiry = DateTime.fromMillisecondsSinceEpoch(data['expires_at'] * 1000);
        await _storage.write(key: 'strava_expiry', value: expiry.toIso8601String());
      }
  ...
  Future<List<ImportedActivity>> _fetchRecentStravaActivities(int limit) async {
    ...
    if (response.statusCode == 200) {
      ...
      for (var activity in data) {
         ...
         activities.add(ImportedActivity(
            ...
            maxPower: activity['max_watts'] == null && activity['weighted_average_watts'] != null 
                ? (activity['weighted_average_watts'] as num).toDouble() // Estimate max from weighted if missing? No, just use weighted as alternative?
                : (activity['max_watts'] as num?)?.toDouble(), // Default to actual max
            // Strava doesn't always send max_watts in summary.
            avgCadence: activity['average_cadence'] as double?,
            calories: (activity['kilojoules'] as num?)?.toInt(), // kJ ~= kcal roughly for cycling (1:1 efficiency assumption commonly used)
            gearId: gearId,
            gearName: gearName,
         ));
      }
  ...
  Future<File?> downloadAndSaveGpx(String activityId, String activityName) async {
    final token = await _storage.read(key: 'strava_access_token');
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('https://www.strava.com/api/v3/activities/$activityId/streams?keys=latlng,altitude,time&key_by_type=true'),
        headers: {'Authorization': 'Bearer $token'},
      );
      ...
      final altitudes = data['altitude'] != null ? data['altitude']['data'] as List : null;
      final times = data['time']['data'] as List; // Seconds from start (usually)
      ...
      for (int i = 0; i < latlngs.length; i++) {
        ...
        if (timeVal > 1000000000) {
           time = DateTime.fromMillisecondsSinceEpoch(timeVal * 1000, isUtc: true);
        }
        ...
        trkSeg.trkpts.add(
          Wpt(
            ...
            ele: altitudes != null && i < altitudes.length ? (altitudes[i] as num).toDouble() : 0.0,
            time: time,
          ),
        );
      }
