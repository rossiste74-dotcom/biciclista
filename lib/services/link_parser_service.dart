    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (var meta in metaTags) {
          if (meta.attributes['property'] == 'og:description' || meta.attributes['name'] == 'description') {
        ...
        final distanceRegExp = RegExp(r'([\d\.,]+)\s*(km|chilometri|kilometers)', caseSensitive: false);
        ...
        final elevRegExp = RegExp(r'([\d\.,]+)\s*(m|metri|elevation|dislivello)', caseSensitive: false);
        ...
        final specificElevRegExp = RegExp(r'(dislivello|elevation|ascenso|elev)[\s:]*([\d\.,]+)\s*m?', caseSensitive: false);
        final specificElevMatch = specificElevRegExp.firstMatch(fullText);
        
        if (specificElevMatch != null) {
           final elevStr = specificElevMatch.group(2)?.replaceAll(',', '.') ?? '';
        ...
        final dateRegExp = RegExp(r'(\d{1,2})[\s/]+([a-zA-Z]{3}|\d{1,2})[\s/]+(\d{4}|\d{2})', caseSensitive: false);
        ...
        int? heartRate;
        final hrRegExp = RegExp(r'([\d]{2,3})\s*(bpm|hr|fc media|frequenza cardiaca)', caseSensitive: false);
        ...
        // Power (W, watt)
        int? power;
        final powerRegExp = RegExp(r'([\d]{2,4})\s*(W|Watt)', caseSensitive: false);
