...
class OCRService {
  Future<Map<String, dynamic>> extractRideDataFromImage(String imagePath) async {
    ...
    try {
      ...
      final elevRegExp = RegExp(r'(dislivello|ascenso|elev|elevation)[\s\n]*([\d\.,]+)\s*m', caseSensitive: false);
      ...
      final timeRegExp = RegExp(r'([\d]+)[h:]\s*([\d]+)[m]?', caseSensitive: false);
      ...
      int? heartRate;
      final hrRegExp = RegExp(r'([\d]{2,3})\s*(bpm|fc media|frequenza cardiaca)', caseSensitive: false);
      ...
      for (var powerMatch in matches) {
        ...
        if (pwr != null && pwr > 30 && pwr < 2000) { // sanity check for power vs other numbers
           power = pwr;
           break;
        }
      }
      ...
      return {
        ...
        'heartRate': heartRate,
        'power': power,
        'notes': 'Importato tramite screenshot.\n${timeStr != null && timeStr.isNotEmpty ? 'Tempo/Data: $timeStr\n' : ''}\nTesto grezzo letto:\n$text',
      };
      
    } catch (e) {
      print('OCR Error: $e');
      return {
        'notes': 'Errore durante la lettura OCR: $e'
      };
    } finally {
