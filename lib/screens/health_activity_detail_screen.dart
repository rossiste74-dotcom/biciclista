...
class HealthActivityDetailScreen extends StatefulWidget {
  ...
  const HealthActivityDetailScreen({super.key, required this.plannedRide});

  @override
  State<HealthActivityDetailScreen> createState() =>
      _HealthActivityDetailScreenState();
}

class _HealthActivityDetailScreenState
    extends State<HealthActivityDetailScreen> {
  ...
  Future<void> _loadActivityDetails() async {
    ...
    try {
      ...
      if (!kIsWeb) {
        try {
          ...
          if (stepsData.isNotEmpty) {
            _steps = stepsData
                .map(
                  (e) => (e.value as NumericHealthValue).numericValue.toInt(),
                )
                .reduce((a, b) => a + b);
          }
        } catch (e) {
      ...
      if (widget.plannedRide.bicycleId != null) {
        ...
        _assignedBicycle = await db.getBicycleById(
          widget.plannedRide.bicycleId!,
        );
      }
                    ...
                    child: Column(
                      ...
                      children: [
                              ...
                              if (_maxHeartRate != null)
                                Expanded(
                                  child: _buildMetricCard(
                                    ...
                                    label: 'FC Max',
                                    value:
                                        '${_maxHeartRate!.toStringAsFixed(0)} bpm',
                          ...
                          if (_assignedBicycle != null)
                            Card(
                              ...
                              child: ListTile(
                                ...
                                title: Text(
                                  _assignedBicycle!.name,
                                  style: const TextStyle(
                                    fontWeight: FontWeight.bold,
                                  ),
                                ),
                                subtitle: Text(
                                  'Bicicletta utilizzata (${_assignedBicycle!.type})',
                                ),
                              ),
                            ),
                          if (_assignedTrack != null)
                        ...
                        if (_routeData != null && _assignedTrack != null) ...[
                          ...
                          if ((_routeData!['climbs'] as List?)?.isNotEmpty ??
                              false) ...[
                            Text(
                              'Salite Impegnative',
                              style: Theme.of(context).textTheme.titleMedium,
                          ...
                          if (_assignedTrack!.description != null &&
                              _assignedTrack!.description!.isNotEmpty) ...[
                            Text(
                              'Analisi Butler AI',
                              style: Theme.of(context).textTheme.titleMedium,
                            ),
  ...
  Future<void> _showCompletionDialog(BuildContext context) async {
    ...
    if (bicycles.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Devi avere almeno una bicicletta nel garage per completare l\'attività.',
          ),
        ),
      );
      return;
  ...
  Future<void> _saveCompletedActivity(String bikeId, String? trackId) async {
    ...
    try {
      ...
      bike.totalKilometers =
          (bike.totalKilometers.isNaN ? 0.0 : bike.totalKilometers) +
          ride.distance;
      bike.chainKms =
          (bike.chainKms.isNaN ? 0.0 : bike.chainKms) + ride.distance;
      ...
      for (var component in bike.components) {
        component.currentKm =
            (component.currentKm.isNaN ? 0.0 : component.currentKm) +
            ride.distance;
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Attività completata e km aggiunti a ${bike.name}!'),
          ),
        );
      }
    } catch (e) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Errore: non è stato possibile completare l\'attività.',
            ),
          ),
        );
      }
