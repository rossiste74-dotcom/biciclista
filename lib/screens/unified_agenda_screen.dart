...
class UnifiedAgendaScreen extends StatefulWidget {
  ...
  const UnifiedAgendaScreen({super.key, this.initialTabIndex = 0});
...
class _UnifiedAgendaScreenState extends State<UnifiedAgendaScreen>
      ...
      if (mounted) {
        setState(() {
          ...
          _upcomingActivities = activities.where((a) {
            ...
            debugPrint(
              'Activity: \${a.rideName} - Time: \$meetingTime vs Now: \$now -> IsFuture? \$isFuture',
            );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ...
          : TabBarView(
              controller: _tabController,
              children: [_buildListTab(), _buildMapTab(), _buildCompletedTab()],
            ),
  ...
  Future<void> _processImportedLink(String url) async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Errore analisi link: $e')));
      }
    }
  }
  ...
  Widget _buildCompletedTab() {
    ...
    allCompleted.sort((a, b) {
      final dateA = (a is GroupRide)
          ? a.meetingTime
          : (a as PlannedRide).rideDate;
      final dateB = (b is GroupRide)
          ? b.meetingTime
          : (b as PlannedRide).rideDate;
  ...
  Widget _buildPersonalActivityCard(PlannedRide ride) {
    ...
    if (nameInput.contains("CYCLING") ||
        nameInput.contains("BIKING") ||
        nameInput.contains("CICLISMO") ||
        nameInput.contains("BICI")) {
      icon = Icons.directions_bike;
      iconColor = Colors.blue;
    } else if (nameInput.contains("WALKING") ||
        nameInput.contains("CAMMINATA")) {
      ...
      child: ListTile(
        ...
        onTap: () {
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) =>
                  HealthActivityDetailScreen(plannedRide: ride),
            ),
  ...
  Widget _buildMapTab() {
    ...
    final mapActivities = _upcomingActivities
    ...
    final center = mapActivities.isNotEmpty
        ? LatLng(
            mapActivities.first.meetingLatitude!,
            mapActivities.first.meetingLongitude!,
          )
