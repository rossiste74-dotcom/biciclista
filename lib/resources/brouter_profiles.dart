const String kMtbProfileTemplate = r'''
...
assign   MTB_factor             {{MTB_FACTOR}} # default 0.0, *)
...
assign   avoid_primary          {{AVOID_PRIMARY}}
assign   avoid_secondary        {{AVOID_SECONDARY}}
...
if mtb:scale=0       then 0.0 else   if mtb:scale=0+      then {{SCALE_0_PLUS_PENALTY}} else
if mtb:scale=1-      then {{SCALE_1_MINUS_PENALTY}} else   if mtb:scale=1       then {{SCALE_1_PENALTY}} else
if mtb:scale=1+      then {{SCALE_1_PLUS_PENALTY}} else   if mtb:scale=2-      then {{SCALE_2_MINUS_PENALTY}} else
if mtb:scale=2       then {{SCALE_2_PENALTY}} else   if mtb:scale=2+      then {{SCALE_2_PLUS_PENALTY}} else
if mtb:scale=3       then {{SCALE_3_PENALTY}} else
...
assign rawcostfactor ( # can be <1, adjusted to >=1 in final step of calculation of costfactor, uphillcostfactor,downhillcostfactor
  ...
  # User custom avoidances
  switch and avoid_primary highway=primary|primary_link 10000
  switch and avoid_secondary highway=secondary|secondary_link 10000
